	...
	while (list_length(initial_rels) > 1)
	{
		...
		foreach(i, initial_rels)
		{
			...
			for_each_cell(j, initial_rels, lnext(initial_rels, i))
			{
				...
				/*
				 * A "base pair" is one where both inputs are single base
				 * relations — cost_edge is accurate here. For composite
				 * rels, use estimated result size instead.
				 */
				...
				{
					/*
					 * Prefer base-pair joins over composite joins: base pairs
					 * have accurate costs and produce smaller intermediates
					 * that improve later joins.
					 */
					best_cost = prel_join_cost;
					best_size = rel1->rows * rel2->rows *
						get_selectivity(root, rel1, rel2);
...
List *
goo_final_cost_final_result(PlannerInfo *root, List *initial_rels,
							bool clauseless, bool force)
{
	while (list_length(initial_rels) > 1)
	{
...
List *
goo_init_result(PlannerInfo *root, List *initial_rels,
				bool clauseless)
{
	while (list_length(initial_rels) > 1)
	{
...
List *
goo_final_result(PlannerInfo *root, List *initial_rels,
				 bool clauseless)
{
	while (list_length(initial_rels) > 1)
	{
...
goo_init_cost(PlannerInfo *root, List *initial_rels,
			  bool clauseless, bool force)	/* TODO cost (1), select(2),
											 * sizes(3+) */
{
	while (list_length(initial_rels) > 1)
	{
...
List *
dummy(PlannerInfo *root, List *initial_rels)
{
	while (list_length(initial_rels) > 1)
	{
		ListCell   *i = NULL;
