 ...
 */
bool has_edge(PlannerInfo *root, RelOptInfo *rel1, RelOptInfo *rel2) {
 ...
 */
bool is_connected(List *all_vertexes, size_t bitmap) {
  List *vertices = NIL;
  bool *used = NULL;
  Vertex *start = NULL;
  size_t topology_id;
 ...
 */
Selectivity get_selectivity(PlannerInfo *root, RelOptInfo *rel1,
                            RelOptInfo *rel2) {
  RelOptInfo joinrel;
...
void set_id(PlannerInfo *root, Topology *topology) {
  ListCell *lc = NULL;
 ...
 */
Cost cost_edge(PlannerInfo *root, RelOptInfo *rel1, RelOptInfo *rel2) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (mergeclauses != NIL) {
      ...
      /*
       * Callers are expected to clear the explicit sort keys when the input
       * path is already ordered.  Be forgiving here in case a caller forgets to
       * do so and avoid redundant work instead of tripping the assertion below.
       */
      if (outersortkeys &&
