...
{
	/*
	 * Bitmap of relations this Hypernode represents. Must be first field in
	 * structure - used as key.
	 */
	bitmapword	set;

	/*
	 * Created 'RelOptInfo' for this HyperNode. During DPhyp algorithm this is
	 * not NULL only for base hypernodes. At the end of algorithm we build
	 * RelOptInfo for all
	 */
	RelOptInfo *rel;

	/*
	 * List of Hypernode pairs, that can contribute for creating this
	 * HyperNode. Used as an indicator that this HyperNode has a plan and can
	 * be created, even if actually 'make_join_rel' will not be able to create
	 * RelOptInfo from it.
	 */
	List	   *candidates;
...
typedef struct DPHypContext
{
	...
	/*
	 * Map of base hypernode to bitmaps containing all nodes that node has
	 * simple edge with
	 */
	bitmapword *simple_edges;

	/*
	 * Array of hyperedges appearing in query graph. Index is id of hypernode
	 * and each array contains all hyperedges that this node appears in
	 */
...
typedef struct SubsetIteratorState
{
	...
	/*
	 * Cache of neighborhoods for different subsets. Indexed by number of
	 * leading zeros in subset.
	 */
